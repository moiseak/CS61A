...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 or len(source_words) == 0:
        return 0.0
    len_ = min(len(typed_words), len(source_words))
    for i in range(len_):
        if typed_words[i] == source_words[i]:
            count_ += 1.0
    return count_ / len(typed_words) * 100.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return 60 / elapsed * len(typed) / 5 * 1.0
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        min_diff = min([diff_function(typed_word, word, limit) for word in word_list])
        if min_diff > limit:
            return typed_word
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    progress = count_ / len(source)
    di = {'id': user_id, 'progress': progress}
    upload(di)
    print(progress)
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    t = []
    for list_ in timestamps_per_player:
        temp = []
        for i in range(len(list_) - 1):
            diff = list_[i + 1] - list_[i]
            temp.append(diff)
        t.append(temp)
    return words, t
    # END PROBLEM 9
...
def time_per_word_match(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    word, times = time_per_word(words, timestamps_per_player)
    match_ = match(word, times)
    return match_
    # END PROBLEM 10
...
def fastest_words(match_object):
    ...
    for word_index in word_indices:
        time = get_time(match_object, 0, word_index)
        word = get_word(match_object, word_index)
        now_player = 0
        result[now_player].append(word)
        for player_index in player_indices:
            new_time = get_time(match_object, player_index, word_index)
            if time > new_time:
                result[now_player].remove(word)
                result[player_index].append(word)
                time = new_time
                now_player = player_index
    return result
    # END PROBLEM 11
