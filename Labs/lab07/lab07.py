...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        count_ = 1
        money = self.balance + self.balance * self.interest
        while money < amount:
            count_ += 1
...
class BankAccount:
    ...
    def deposit(self, amount):
        ...
        self.balance = self.balance + amount
        a = self.balance
        dep = Transaction(self.id, b, a)
        self.transactions.append(dep)
        self.id += 1
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        before = self.balance
        if amount > self.balance:
            no_change = Transaction(self.id, self.balance, self.balance)
            self.transactions.append(no_change)
            self.id += 1
            return 'Insufficient funds'
        self.balance = self.balance - amount
        after = self.balance
        dep = Transaction(self.id, before, after)
        self.transactions.append(dep)
        self.id += 1
        return self.balance
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        for coin in result:
            self.coins = remove_one(self.coins, coin)
        return result
