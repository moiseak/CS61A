...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        while bindings[exp] in bindings:
            exp = bindings[exp]
        return bindings[exp]
    elif exp.first in bindings:  # CHANGE THIS CONDITION FOR Q4 where are variables stored?
        exp.first = bindings[exp.first]
        return calc_eval(exp)  # UPDATE THIS FOR Q4, how do you access a variable?
...
def eval_define(expressions):
    ...
    """
    "*** YOUR CODE HERE ***"
    while expressions.rest.rest is not nil:
        expressions = expressions.rest
    bindings[expressions.first] = expressions.rest.first
    return f'{expressions.first}'
